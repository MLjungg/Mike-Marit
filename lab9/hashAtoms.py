...
class Hashtable:
    ...
    def store(self, node):  # Hashar in min node
        index = self.hashingFunction(node.key)

        if self.table[index] == None:
            self.table[index] = LinkedQ()  # skapar en länkadlista på platsen
            self.table[index].enqueue(node)
        else:
            self.table[index].enqueue(node)
...
def loadfactor(antalElement): #Loadfactor = 0.50 för att få tillräckligt med luft i listan.
    size = int(antalElement/0.50)
    return size
